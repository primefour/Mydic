...
GZipHeader::GZipHeader(const char *gzip_path){
    ...
    Sobj<SimpleFile> file_obj = NULL;
    try{
        file_obj = new SimpleFile(gzip_path,O_RDONLY);
    }catch (exception & a){
        angel_printfd(" EXCEPTION %s \n",a.what());
...
void GZipHeader::getExtraInfo(int &chunk_num,int &chunk_len,int & version){
    if(mExtraBuff != NULL){
        ...
        ptr += 2;
        angel_printfi("ver = %u chlen = %u chcnt = %u \n",ver,chlen,chcnt);
        chunk_num = chcnt;
        chunk_len = chlen;
        version = ver;
    }
