			     ...
			     dictData *header, int computeCRC )
{
   ...
   if (id1 != GZ_MAGIC1 || id2 != GZ_MAGIC2) {
	 ...
	 while (!feof( str )) {
	    if ((count = fread( buffer, 1, BUFFERSIZE, str ))) {
	       crc = crc32( crc,(const Bytef*) buffer, count );
	    }
	 }
      }
      header->crc = crc;
   ...
   if (header->flags & GZ_FEXTRA) {
	 ...
	 header->chunks =(int*) xmalloc( sizeof( header->chunks[0] ) * header->chunkCount );
   ...
   header->compressedLength = ftell( str );
				...
				/* Compute offsets */
   header->offsets = (long unsigned int*) xmalloc( sizeof(header->offsets[0] ) * header->chunkCount );
...
dictData *dict_data_open( const char *filename, int computeCRC )
{
   ...
   h = (dictData*)xmalloc( sizeof( struct dictData ) );
   ...
   case DICT_DZIP:
	 ...
	 if (found) {
	    ...
	    if (!h->cache[target].inBuffer)
	       h->cache[target].inBuffer = (char *)xmalloc( IN_BUFFER_SIZE );
	    inBuffer = h->cache[target].inBuffer;
